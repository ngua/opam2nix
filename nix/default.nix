...
, sources
, ocaml-ng
, ocamlPackagesOverride ? ocaml-ng.ocamlPackages_4_13
}:
let
...
let
  ...
  callOcamlPackage = ocamlPackages.newScope {
    ...
    zeroinstall-solver = callOcamlPackage
      ({ buildDunePackage }:
        buildDunePackage {
          useDune2 = true;
    ...
    opam-0install = callOcamlPackage
      ({ buildDunePackage, fmt, cmdliner, opam-state, zeroinstall-solver }:
        buildDunePackage {
          pname = "opam-0install";
    ...
    opam-file-format = callOcamlPackage
      ({ buildDunePackage }:
        buildDunePackage {
    ...
    spdx_licenses = callOcamlPackage
      ({ buildDunePackage }:
        buildDunePackage {
          pname = "spdx_licenses";
          version = "main";
...
in
callOcamlPackage ./opam2nix.nix { opam2nixSrc = ../.; }
